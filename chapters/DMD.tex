...
\section{Dynamic Mode Decomposition (DMD) \cite{SlaySlaySlay}}
	...
	\begin{align*}
		...
		\end{pmatrix}                                                                                              \\
		     & = \begin{pmatrix}
		         ...
		         \end{pmatrix}
		= \begin{pmatrix}
			  \displaystyle\sum_{j=1}^{n}\bar{x}_{1j} x_{1j} &        & *                                                    \\
			                                                      & \ddots &                                                      \\
			  *                                                   &        & \displaystyle\sum_{j=1}^{n} \bar{x}_{mj} x_{mj}
		  \end{pmatrix}\,.
	\end{align*}

	Mit \(\bar{x}x = |x|^2\) erhalten wir
	\begin{align}
		\tr \left(X^HX\right) = \sum_{i=1}^{m}\sum_{j=1}^{n} \bar{x}_{ij} x_{ij} = \sum_{i=1}^{m}\sum_{j=1}^{n} |x_{ij}|^2 = ||X||_F^2 \,. \label{eq:traceFrob}
	\end{align}
\end{proof}
...
\begin{proof}\textcolor{white}{Wackelpudding} %\emph{Beweis.}
	\begin{enumerate}
		\item Aus Gleichung \eqref{eq:traceFrob} folgt
		      \[
			      ||X||_F^2 = \tr \left(X^HX\right) = \sum_{i=1}^{m}\sum_{j=1}^{n} \bar{x}_{ij} x_{ij} = \sum_{j=1}^{n}\sum_{i=1}^{m} \bar{x}_{ij} x_{ij} = \tr\left(XX^H\right) = ||X^H||_F^2 \,.
		      \]
		\item Es ist
		      \[
