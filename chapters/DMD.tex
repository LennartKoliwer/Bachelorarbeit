...
Sei nun \(X\in\mathbb{R}^{p\times n}\) der Zustand des Systems zu \(m \) diskreten Zeitpunkten \(t_k =k \Delta t\)  mit \(k\in\NN\) also
\[
	X=\begin{pmatrix}
		|      & |      &       & |      \\
	...
	\end{pmatrix}
\]
und \(X'\in\mathbb{R}^{p\times n}\) der Zustand mit jedem Punkt einen Zeitschritt weiter mit \(t'_k=t_k +\Delta t\)
\[
	X'=\begin{pmatrix}
		|       & |       &       & |       \\
...
Dabei sind die hier betrachteten Pseudoinversen alle Moore-Penrose-Inverse. Um die zu bestimmen, machen wir eine Singulärwertzerlegung von \(X\).
