...
(In dieser Arbeit beschäftigen wir uns mit einer Möglichkeit, den Rechenaufwand des Dynamic Mode Decomposition Algorithmus, kurz DMD, zu verringern.)
DMD ist ein Algorithmus, der ursprünglich im Feld der Strömungsdynamik entwickelt wurde.
Er dient dazu, anhand von Messdaten mehrerer Zeitpunkte die Dynamik eines Systems zu approximieren.

Im Laufe der letzten Jahre wurden eine Vielzahl von Varianten der DMD Algorithmen entwickelt.
...
Bei der Singulärwertzerlegung (SVD) finden wir für ein $X\in\RR^{n\times m}$ zwei orthogonale Matrizen $U$ und $V$ mit passenden Dimensionen und ${\hat{\Sigma}=\diag(\sigma_1,\dots\sigma_r)}$ mit $r\leq\min(n,m)$ und $\sigma_1 \geq \sigma_2 \geq \dots \geq \sigma_r > 0$ so, dass
\[
...
Um für die folgenden Schritte der DMD den Rechenaufwand zu minimieren, lohnt es sich eine Dimensionsreduktion durchzuführen. Eine Möglichkeit ist unter ökonomischer SVD (TSVD) bekannt.
...
\textcolor{red}{Grobe richtung} Im Paper \cite{OHT} von Matan Gavish and David L. Donoho erhalten wir durch \textcolor{red}{...}, dass noise-freie Datenmatrizen ab einem Singulärwert von $\frac{4}{\sqrt{3}}$ abgeschnitten wird. \textcolor{red}{...}
...
Bei kleinen Matrizen, ist die Noise kaum ein Problem aber für ''große'' Matrizen kann der Noiseanteil der Singulärwerte sehr groß werden. 

Ziel ist die Rekonstruktion von $\Xtrue$ aus $X$, insbesondere bei großem $n,m$
